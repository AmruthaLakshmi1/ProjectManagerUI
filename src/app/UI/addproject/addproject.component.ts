...
config = {
  ...
  $('#btntaskmanagersearch').click(function() {
   // console.log("searchclicked");
    console.log($('#btntaskmanagersearch').val());
    $('#searchmodal').show();
  });
  });
}
...
Edit(id:number){
    ...
    .subscribe(i=>{this.itemuser=i
      ...
      console.log(this.itemuser.FirstName); 
      }    
    }); 
    });  
}
...
updaterecord(){
  ...
  this._title.setTitle("ADD PROJECT");
  console.log("enddate"+$('#ptxtenddate').val()+this.st1);
  console.log("enddate"+$('#ptxtenddate1').val()+this.st);
...
//         this.item.PStartDate = $('#ptxtstartdate').val();
//         this.item.PEndDate = $('#ptxtenddate').val();
//         this.item.Manager=this.objarray[0].FirstName;
